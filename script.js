...
// REEMPLAZA CON TU URL REAL DEL WEB APP DESPUÉS DE DESPLEGARLO
const GOOGLE_SHEETS_WEB_APP_URL = 'TU_URL_DEL_WEB_APP_AQUI';

/**
 * Función para enviar datos a Google Sheets
 * Se ejecuta automáticamente después de generar el PDF para planillas específicas
 */
PTGApp.prototype.sendDataToGoogleSheets = async function(formData) {
    if (!this.sheetsIntegrationEnabled) {
        console.log('Integración Google Sheets deshabilitada');
        return;
    }

    // Solo enviar para los tipos de equipos específicos
    const sheetsCompatibleTypes = [
    ...
    if (!sheetsCompatibleTypes.includes(formData.equipmentType)) {
        console.log(`Tipo ${formData.equipmentType} no compatible con integración Google Sheets`);
        return;
    }

    try {
        this.showSheetsStatus('Enviando datos a Google Sheets...', 'loading');
        ...
        const response = await fetch(GOOGLE_SHEETS_WEB_APP_URL, {
            ...
            body: JSON.stringify({
                ...
                equipmentData: this.getEquipmentSpecificData(formData)
            })
        });
        ...
        if (result.success) {
            this.showSheetsStatus(`✅ Datos guardados en Google Sheets`, 'success');
        ...
        this.showSheetsStatus(`❌ Error al enviar a Google Sheets: ${error.message}`, 'error');
    }
};

/**
 * Extraer datos específicos del equipo según el tipo
 */
PTGApp.prototype.getEquipmentSpecificData = function(formData) {
    const data = {};

    switch(formData.equipmentType) {
        case 'Planillas Datos Frick K-401':
            data.frick = formData.frick;
            break;
        case 'Planillas Datos Motocompresores':
            data.compressors = formData.compressors;
            break;
        case 'Planillas Datos Recompresores':
            data.recompressors = formData.recompressors;
            break;
        case 'Planillas Datos TurboExpander':
            data.turboExpanderData = formData.turboExpanderData;
            break;
    }
...
/**
 * Mostrar estados de la integración con Google Sheets
 */
PTGApp.prototype.showSheetsStatus = function(message, type = 'info') {
    // Crear o actualizar el elemento de estado
    let statusDiv = document.getElementById('sheetsStatus');
    
    if (!statusDiv) {
        statusDiv = document.createElement('div');
        statusDiv.id = 'sheetsStatus';
    ...
    switch(type) {
        ...
        case 'error':
            ...
            break;
        default:
            statusDiv.style.backgroundColor = '#34495e';
            statusDiv.innerHTML = message;
            break;
    }

    // Agregar animación CSS si no existe
    if (!document.getElementById('sheetsStatusCSS')) {
        const style = document.createElement('style');
        style.id = 'sheetsStatusCSS';
        style.textContent = `
            @keyframes spin {
 ...
 */
PTGApp.prototype.testGoogleSheetsConnection = async function() {
    try {
        this.showSheetsStatus('Probando conexión con Google Sheets...', 'loading');
        
        const testData = {
            equipmentType: 'Test Connection',
        ...
        if (response.ok) {
            this.showSheetsStatus('✅ Conexión exitosa con Google Sheets', 'success');
        } else {
        ...
        this.showSheetsStatus(`❌ Error de conexión: ${error.message}`, 'error');
    }
};

/**
 * Función para habilitar/deshabilitar la integración Google Sheets
 */
PTGApp.prototype.toggleSheetsIntegration = function() {
    this.sheetsIntegrationEnabled = !this.sheetsIntegrationEnabled;
    const status = this.sheetsIntegrationEnabled ? 'habilitada' : 'deshabilitada';
    this.showSheetsStatus(`Integración Google Sheets ${status}`, 'info');
    console.log(`Integración Google Sheets: ${status}`);
};

/**
 * Agregar botón de prueba para desarrollo
 */
PTGApp.prototype.addSheetsTestButton = function() {
    // Solo en desarrollo
    ...
    testButton.innerHTML = '🧪 Test Sheets';
    ...
    // Agregar botón toggle también
    const toggleButton = document.createElement('button');
    toggleButton.innerHTML = '🔄 Toggle Sheets';
 ...
 */
PTGApp.prototype.validateSheetsData = function(formData) {
    const errors = [];

    if (!formData.equipmentType) {
        errors.push('Tipo de equipo requerido');
    }

    if (!formData.operatorName || formData.operatorName.trim() === '') {
        errors.push('Nombre del operador requerido');
    }
    ...
    // Validaciones específicas por tipo de equipo
    switch(formData.equipmentType) {
        case 'Planillas Datos Frick K-401':
            if (!formData.frick || !formData.frick.equipment) {
                errors.push('Datos Frick incompletos');
            }
            break;
        case 'Planillas Datos Motocompresores':
            if (!formData.compressors || Object.keys(formData.compressors).length === 0) {
                errors.push('Datos de Motocompresores incompletos');
            }
            break;
        case 'Planillas Datos Recompresores':
            if (!formData.recompressors || Object.keys(formData.recompressors).length === 0) {
                errors.push('Datos de Recompresores incompletos');
            }
            break;
        case 'Planillas Datos TurboExpander':
            if (!formData.turboExpanderData) {
                errors.push('Datos de TurboExpander incompletos');
            }
            break;
    }
...
/**
 * Función mejorada de envío con validación
 */
PTGApp.prototype.sendDataToGoogleSheetsWithValidation = async function(formData) {
    if (!this.sheetsIntegrationEnabled) {
        console.log('Integración Google Sheets deshabilitada');
        return;
    }
    ...
    if (!GOOGLE_SHEETS_WEB_APP_URL || GOOGLE_SHEETS_WEB_APP_URL === 'https://script.google.com/macros/s/AKfycby1YUaIcdpf21MQjuvsi3DbHMM0x1NXMcOdB7PUo1E8WJWv7PLguJAJsnO9MrSilR8e/exec') {
        console.error('URL de Google Sheets Web App no configurada');
        this.showSheetsStatus('❌ URL de Google Sheets no configurada', 'error');
        return;
    }

    // Validar datos
    const validationErrors = this.validateSheetsData(formData);
    if (validationErrors.length > 0) {
        console.error('Errores de validación:', validationErrors);
        this.showSheetsStatus(`❌ Errores: ${validationErrors.join(', ')}`, 'error');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            this.showSheetsStatus(`✅ Datos guardados en Google Sheets`, 'success');
            console.log('Resultado exitoso de Google Sheets:', result);
        } else {
        ...
        if (error.name === 'TypeError' && error.message.includes('fetch')) {
            this.showSheetsStatus('❌ Error de conexión - Verificar URL del Web App', 'error');
        } else {
            this.showSheetsStatus(`❌ Error al enviar: ${error.message}`, 'error');
        }
    }
};
 ...
 */
PTGApp.prototype.showSheetsConfiguration = function() {
    console.log('=== CONFIGURACIÓN GOOGLE SHEETS ===');
    console.log('URL Web App:', GOOGLE_SHEETS_WEB_APP_URL);
    console.log('Integración habilitada:', this.sheetsIntegrationEnabled);
    console.log('Tipos compatibles:', [
        'Planillas Datos Frick K-401',
        'Planillas Datos Motocompresores', 
        'Planillas Datos Recompresores',
        'Planillas Datos TurboExpander'
    ]);
    console.log('Nombres de hojas actualizados:');
    console.log('- Frick: Días del mes (1, 2, 3... 31)');
    console.log('- Motocompresores: MC#1, MC#2, MC#3, MC#5, MC#6');
    console.log('- Recompresores: RC #4, RC #7, RC #8, RC #9');
    console.log('- TurboExpander: 2025');
    console.log('=== FIN CONFIGURACIÓN ===');
};

/**
 * Función para simular envío de datos (testing)
 */
PTGApp.prototype.simulateDataSend = function(equipmentType = 'Planillas Datos Frick K-401') {
    const mockData = {
        ...
        dateTime: new Date().toISOString(),
        frick: {
            equipment: 'Frick K-401A',
            frickSuctionPressure: '5.2',
            frickSuctionTemp: '15.5',
            frickDischargePressure: '12.8',
            frickDischargeTemp: '85.2'
        }
    };

    console.log('Simulando envío de datos:', mockData);
    this.sendDataToGoogleSheetsWithValidation(mockData);
};
...
class PTGApp {
    constructor() {
        ...
        // Integración Google Sheets
        this.sheetsIntegrationEnabled = true;
    ...
    init() {
        ...
        // Inicializar integración Google Sheets
        this.sheetsIntegrationEnabled = true;

        // Agregar botones de prueba en desarrollo
        if (window.location.hostname === 'localhost' || window.location.hostname.includes('github.io')) {
            this.addSheetsTestButton();
        }
    }
    ...
    handleEquipmentTypeChange(event) {
        ...
        this.hideAllFields();
        this.currentEquipmentType = selectedValue;
    ...
    collectRecompressorsData() {
        ...
        units.forEach(unit => {
            ...
            // RC9 tiene campos específicos diferentes
            if (unit === 'rc9') {
                const rc9Fields = [
    ...
    async generatePDF() {
        ...
        try {
            ...
            // Enviar a Google Sheets (nuevo)
            try {
                const data = this.collectFormData();
                await this.sendDataToGoogleSheetsWithValidation(data);
            } catch (sheetsError) {
    ...
    addOilDataToPDF(doc, data, yPosition, lineHeight) {
        ...
        Object.entries(data).forEach(([unitKey, unitData]) => {
            if (unitNames[unitKey]) {
                ...
                doc.setFont('helvetica', 'normal');
                if (unitData.motorCm) doc.text(`Nivel Lado Motor: ${unitData.motorCm} cm`, 25, yPosition), yPosition += line