...
class PTGApp {
    ...
    collectRecompressorsData() {
        ...
        units.forEach(unit => {
            recompressors[unit] = {};
            // Collect all recompressor fields
            const fields = [
    ...
    async generatePDF() {
        ...
        try {
            ...
            // Enviar datos a Google Sheets después de generar el PDF
            generateBtn.textContent = 'Enviando datos a planillas...';
            
            try {
                const sheetResponse = await this.sendDataToGoogleSheets(data);
                if (sheetResponse && sheetResponse.success) {
                    alert('PDF generado y datos enviados exitosamente a Google Sheets');
                } else {
                    alert('PDF generado pero hubo un problema al enviar datos a Google Sheets');
                }
            } catch (sheetError) {
                console.error('Error enviando a Google Sheets:', sheetError);
                alert('PDF generado pero hubo un error al enviar los datos a Google Sheets');
            }
    ...
    async sendDataToGoogleSheets(data) {
        try {
            // Solo enviar datos para tipos de equipo que tienen planillas
            const sheetCompatibleTypes = [
            ...
            if (!sheetCompatibleTypes.includes(data.equipmentType)) {
                console.log('Tipo de equipo no requiere integración con planillas:', data.equipmentType);
                return {success: true, message: 'No se requiere integración con planillas'};
            }
            ...
            if (data.equipmentType === 'Planillas Datos Frick K-401') {
                postData.equipmentData.frick = data.frick;
            } else if (data.equipmentType === 'Planillas Datos Motocompresores') {
                postData.equipmentData.compressors = data.compressors;
            } else if (data.equipmentType === 'Planillas Datos Recompresores') {
                postData.equipmentData.recompressors = data.recompressors;
            } else if (data.equipmentType === 'Planillas Datos TurboExpander') {
                postData.equipmentData.turboExpanderData = data.turboExpanderData;
            }
            
            console.log('Enviando datos a Google Sheets:', postData);
            
            // Enviar datos a App Script usando XMLHttpRequest para mejor compatibilidad
            return new Promise((resolve, reject) => {
                const xhr = new XMLHttpRequest();
                xhr.open('POST', this.appScriptUrl, true);
                ...
                xhr.onload = function() {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        try {
                            ...
                            console.log('Respuesta recibida (posiblemente texto simple):', xhr.responseText);
                            resolve({
                                success: true,
